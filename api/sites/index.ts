...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      const site = req.body;
      if (!site || typeof site !== 'object') return res.status(400).json({ error: 'Request body missing.' });
      if (!site.name?.trim()) return res.status(400).json({ error: 'Site name is required.' });
